...
let mistral;
const api = "YgHPIlQBYw7Ij2dzW21j3R8aOV4mNNrq";
console.log(api);
try {
  mistral = new Mistral({
    apiKey: api ?? "",
...
export default function HomePage() {
  ...
  const handleTasksUpdate = useCallback(
    (updatedTaskOrTasks) => {
      ...
      } else if (typeof updatedTaskOrTasks === "object" && updatedTaskOrTasks !== null) {
        updatedTasks = tasks.map((task) =>
          task._id === updatedTaskOrTasks._id ? updatedTaskOrTasks : task
        );
      } else {
        console.error("handleTasksUpdate received invalid data:", updatedTaskOrTasks);
        return;
      }
  ...
  const sendToMistral = async (prompt) => {
    ...
    try {
      ...
      setMistralOutput(prevOutput => prevOutput + "\n\nMistral: " + result.choices[0].message.content);
    } catch (error) {
  ...
  return (
    <div className={`w-full min-h-screen bg-${isDarkMode ? "darkMode-background" : "white"}`}>
      <div className="grid grid-cols-4 grid-rows-6 h-screen gap-4 p-4">
        ...
        <div className="col-start-1 col-end-2 row-start-1 row-end-2 flex items-center justify-between p-4">
          <div className="flex items-center">
            ...
            <h1 className="text-2xl font-regular text-gray-900 dark:text-white">Petal Notes</h1>
          </div>
          <ThemeToggleButton />
        </div>
        ...
        <div className="col-start-2 col-end-3 row-start-1 row-end-2 bg-lightMode-background dark:bg-black p-4 flex justify-between items-center rounded-2xl">
          <div className="text-black dark:text-white text-xl">
            {"  Good "}
            {checkTime(currentHour)}, {currentUser ? currentUser.username || currentUser.email : "Guest"}
          </div>
          <div className="flex items-center">
            <button
        ...
        {/* Mistral Chat */}
        <div ref={chatContainerRef} className="col-start-2 col-end-5 row-start-4 row-end-6 bg-lightMode-topTask dark:bg-darkMode-topTask rounded-2xl p-4 overflow-y-auto">
          <div className="text-black dark:text-white text-left ">
            <pre className="whitespace-pre-wrap">{mistralOutput}</pre>
          </div>
        </div>
